...
void bncUploadCaster::open() {
  ...
  if (!_outSocket->waitForConnected(timeOut)) {
    delete _outSocket;
    _outSocket = 0;
    emit(newMessage("Broadcaster: Connect timeout for " + _mountpoint.toAscii()
         + " (" + _outHost.toAscii() + ")", true));
    return;
  }
  ...
  if (ans.indexOf("OK") == -1) {
    delete _outSocket;
    _outSocket = 0;
    emit(newMessage("Broadcaster: Connection broken for " + _mountpoint.toAscii()
         + " (" + _outHost.toAscii() + ")", true));
  }
  else {
    emit(newMessage("Broadcaster: Connection opened for " + _mountpoint.toAscii()
         + " (" + _outHost.toAscii() + ")", true));
    _sOpenTrial = 0;
  }
}
