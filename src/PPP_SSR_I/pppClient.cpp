...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  if (_filter->update(_epoData) == success) {
    ...
    output->_tempStr     = _filter->tempStr();
    output->_tempDb      = _filter->tempDb();
    output->_xyz[0]      = _filter->xyz()[0];
    output->_xyz[1]      = _filter->xyz()[1];
    output->_xyz[2]      = _filter->xyz()[2];      
  }
  else {
    output->_error = true;
  }
