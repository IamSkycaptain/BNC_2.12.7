...
gtime_t bncTimeTogtime(const bncTime& time){
	std::string datetime = time.datestr() + " " + time.timestr(3);
	char* T = strdup(datetime.c_str());	
		
	time_t timer;
	gtime_t gtime;
	int year = 0, month = 0, day = 0, hour = 0, min = 0;
	double sec = 0.0;
	//2016-11-10 00:02:54.000
	sscanf(T, "%4d-%2d-%2d %2d:%2d:%lf", &year, &month, &day, &hour, &min, &sec);
